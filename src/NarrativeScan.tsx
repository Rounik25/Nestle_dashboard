  ...
  return (
    <div className="flex h-full w-full min-h-0 min-w-0 flex-col">
      ...
      <div
        className={
          view
            ? "grid h-full min-h-0 w-full min-w-0 grid-cols-1 auto-rows-[100%] gap-20 py-10 overflow-y-auto px-80"
      ...
      >
        {cards.map((card, index) => (
          <div
            key={index}
            className={`flex min-h-0 flex-col rounded-2xl bg-white px-5 py-2 shadow-lg ${
              view ? "h-full" : ""
            }`}
          >
            ...
            <div className={`${card.contentClassName} min-h-0 flex-1`}>
              {card.content}
            </div>
          </div>
        ))}
